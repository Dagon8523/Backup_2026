...
- Forward-fill en TODAS las columnas excepto SUBSISTEMA_A_LAS_QUE_PERTENECE_EL_RIESGO.
...
- %: Detecta columnas de porcentaje y normaliza a número 0..100 con 2 decimales.
- Renombrado final de columnas en el orden indicado.
- Exporta a Excel usando xlsxwriter si existe, y si no, openpyxl.
"""
...
INPUT_XLSX = BASE / "Matriz_de_Riesgos_CONSOLIDADA_FINAL.xlsx"
SHEET_NAME = "Matriz de Riesgos"
HEADER_ROW_NUMBER = 2
OUTPUT_XLSX = BASE / "matriz_riesgos_procesada.xlsx"

PRESERVAR_ENYE = False
...
COLUMNAS_FINALES = [
    ...
    "CALIFICACION_DE_PROBABILIDAD_4","NIVEL_DE_RIESGO_RESIDUAL","NO_CONTROL","DESCRIPCION_DEL_CONTROL","RESPONSABLE_DEL_CONTROL",
    ...
    "NIVEL_DE_RIESGO_INHERENTE","OPCION_DE_TRATAMIENTO","ACCIONES_PARA_MITIGAR_EL_RIESGO_","FECHA_DE_EJECUCION","RESPONSABLE",
    "EFICACIA_DE_LAS_ACCIONES_SINO","SEGUIMIENTO_1","FECHA_1","SEGUIMIENTO_2","FECHA_2","SEGUIMIENTO_3","FECHA_3",
    "SEGUIMIENTO_4","FECHA_4","SEGUIMIENTO_5","FECHA_5","SEGUIMIENTO_6","FECHA_6"
]
...
def _normalize_headers(headers: List[str]) -> List[str]:
    ...
    for h in headers:
        ...
        h2 = re.sub(r"[^a-z0-9_% ]", "", h2).replace(" ", "_")
        if not h2:
...
def autosize_columns(writer, df: pd.DataFrame, sheet_name: str, engine: str):
...
def main():
    ...
    col_desc = find_col(df.columns.tolist(), [
        r"^DESCRIPCION_*DEL_*CONTROL$",
        r"^DESCRIPCION_*CONTROL$",
        r".*DESCRIPCION.*CONTROL.*"
    ])
    if not col_desc:
    ...
    if col_subs and mask_subs_vacio is not None:
        df.loc[mask_subs_vacio, col_subs] = pd.NA
    ...
    # NO eliminar columnas vacías: hacen parte de la estructura
    cols_sin_nombre = [c for c in df.columns if c.startswith("COLUMNA_SIN_NOMBRE")]
    if cols_sin_nombre:
        df = df.drop(columns=cols_sin_nombre)
        print(f"[INFO] Columnas sin nombre eliminadas: {cols_sin_nombre}")

    print(f"[INFO] Columnas antes limpieza final: {len(cols_antes)} | después: {len(df.columns)}")
    ...
    print(f"[INFO] Engine de exportación seleccionado: {engine}")
    ...
    with pd.ExcelWriter(OUTPUT_XLSX, engine=engine) as writer:
        ...
        autosize_columns(writer, df, sheet_out, engine)
    ...
    if col_subs and "SUBSISTEMA_A_LAS_QUE_PERTENECE_EL_RIESGO" in df.columns:
